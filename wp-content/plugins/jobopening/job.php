...
//this is the callback function of cpt metabox
function skills_cpt_function($post)
{
	wp_nonce_field(basename(__FILE__), "skills_cpt_nonce");
	?>
	<!-- <p>This is custom owt metabox for custom post type</p> -->
	<div>
		<label for="txtskill">Required Skills</label>
		<?php 
		$skills=get_post_meta($post->ID, "skills_title_name", true);
		?>
		<textarea rows="5" name="txtskill" value="<?php echo $skills; ?>" placeholder="Required Skills" ><?php echo $skills; ?></textarea>
	</div>
	<?php
}
...
function skills_save_metabox_data($post_id, $post)
{
	...
	//we have verified the nonce
	if(!isset($_POST['skills_cpt_nonce'])|| !wp_verify_nonce($_POST['skills_cpt_nonce'],basename(__FILE__))){
		return $post_id;
	}
...
//save value to db field
	...
	{
		$skills_name=sanitize_text_field($_POST['txtskill']);
	}
	else{
		$skills_name='';
	}
	update_post_meta($post_id, "skills_title_name", $skills_name);
}
...
{
	wp_nonce_field(basename(__FILE__), "qualification_cpt_nonce");
	?>
	<!-- <p>This is custom owt metabox for custom post type</p> -->
	<div>
		<label for="txtqualification">Qualifications</label>
		<?php 
		$qualification=get_post_meta($post->ID, "qualification_title_name", true);
		?>
		<textarea rows="5" name="txtqualification" value="<?php echo $qualification; ?>" placeholder="Required Qualifications" ><?php echo $qualification; ?></textarea>
	</div>
	<?php
}
...
function qualifications_save_metabox_data($post_id, $post)
{
	...
	//we have verified the nonce
	if(!isset($_POST['qualification_cpt_nonce'])|| !wp_verify_nonce($_POST['qualification_cpt_nonce'],basename(__FILE__))){
		return $post_id;
	}
...
//save value to db field
	...
	{
		$qualification_name=sanitize_text_field($_POST['txtqualification']);
	}
	else{
		$qualification_name='';
	}
	update_post_meta($post_id, "qualification_title_name", $qualification_name);
}
...
function emptype_cpt_function($post)
{
	...
	<!-- <p>This is custom owt metabox for custom post type</p> -->
	<div>
		<label for="txtemptype">Employee Type</label>
		<?php 
		$emptype=get_post_meta($post->ID, "emptype_title_name", true);
		?>
		<textarea rows="5" name="txtemptype" value="<?php echo $emptype; ?>" placeholder="Employee Type" ><?php echo $emptype; ?></textarea>
	</div>
	<?php
}
